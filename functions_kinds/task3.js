...
function checkOrder(available) {
  readline.question(
    "Please enter the number of nails to order: ",
    (ordered) => {
      if (ordered <= available) {
        nails = available - ordered;
        console.log(
          `You ordered ${ordered} nails. We have ${available} nails left.`,
        );
        orderLoop();
      } else {
        console.log(
          `Sorry, we don't have enough nails. We have ${available} nails left.`,
        );
        channgeOrder();
      }
    },
