...
function gameStart() {
  ...
  console.log(
    chalk.underline.bgRed("This is for the testing purpouse") +
      chalk.bgWhiteBright.red("ONLY!"),
  );
  encounterEnemy();
}
...
function encounterEnemy() {
  ...
  while (enemyHealth > 0 && player.health > 0) {
    let action = readlineSync.question(
      chalk.grey.green("Do you want to attack(a), heal(h) or run(r)?"),
    );
    ...
    switch (action.toLowerCase()) {
      case "a":
        ...
        console.log(
          chalk.blackBright.bgWhiteBright(`Enemy health is now ${enemyHealth}`),
        );
        if (enemyHealth > 0) {
    ...
    if (player.health <= 0) {
      ...
      player.lives -= 1;
      console.log(
        chalk.blackBright.bgWhiteBright(`You have ${player.lives} lives left`),
      );
      if (player.lives <= 0) {
    ...
    if (enemyHealth <= 0) {
      ...
      player.score += 1;
      console.log(
        chalk.blackBright.bgWhiteBright(`Your score is now ${player.score}`),
      );
      roomsVisited += 1;
      console.log(
        chalk.blackBright.bgWhiteBright(
...
function enterNextRoom() {
  if (player.health > 0) {
    let go = readlineSync.question(
      chalk.grey.green("Do you want to go to the next room (y/n)?"),
    );
    if (go.toLowerCase() === "y") {
      ...
      console.log(
        chalk.redBright.bgWhiteBright("Hesitation is critical, soldier!"),
      );
      player.health -= 0;
      console.log(
        chalk.blackBright.bgWhiteBright(`Your health is now ${player.health}`),
      );
      confirmDeath();
    }
  }
